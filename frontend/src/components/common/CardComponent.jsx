...
function CardComponent({id,image,name,description,price,stock,category,deleteProduct,editData,setOpenForm,openForm,setEditForm}) {
  ...
  return (
    <Card className='flex flex-col px-1 gap-y-3 bg-gray-100 shadow-black shadow-xl rounded-2xl border-gray-100 border-4 drop-shadow-xl hover:bg-white hover:border-white cursor-pointer aspect-[16/9]'>
        <img src={image} loading='lazy' alt={name} className='w-full h-full md:h-32 lg:h-40 mx-auto object-contain mb-2'/>
      <CardHeader>
      ...
      <CardFooter className='flex justify-between gap-x-2 mt-1'>
        <Button onClick={() => !openForm && editFormHandler({id,image,name,description,price,stock,category})} className='text-sm py-3 px-7 cursor-pointer'>Edit <MdModeEdit/></Button>
